<template>
  <div>
    <warning-bar
      title="接口调用：https://www.sztv.com.cn/ysztv/web/extends/get?appId="
    />
        ...
        >
          <template #default="scope">
            ...
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateSysExtendsFunc(scope.row)"
            >变更</el-button>
            <el-button
              type="primary"
              link
...
const getTableData = async() => {
  const table = await getSysExtendsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    const res = await deleteSysExtendsByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
...
// 更新行
const updateSysExtendsFunc = async(row) => {
  const res = await findSysExtends({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteSysExtendsFunc = async(row) => {
  const res = await deleteSysExtends({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
